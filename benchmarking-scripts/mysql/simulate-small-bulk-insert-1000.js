...
function generateObservation(baseTimestamp, intervalSeconds) {
...
async function run() {
  const baseTime = new Date(); // Start time now
  const interval = 60; // Simulate 1 observation per 60 seconds
  ...
  try {
    ...
    const observations = generateObservation(baseTime, interval);
    ...
    console.log(`+++ Inserted ${inserted.length} observations.`);
  } catch (error) {
